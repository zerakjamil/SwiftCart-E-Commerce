                    ...
                    @forelse($product->images ?? [] as $image)
                        <div class="swiper-slide">
                            <a href="{{ route('shop.show', $product->slug) }}">
                                <img loading="lazy" src="{{ asset('uploads/products/'. $image['full']) }}"
            ...
            <div class="product-card__price d-flex flex-column align-items-start">
                @if($product->isOnSale())
                    <div class="d-flex align-items-center mb-1">
                        ...
                        <span class="fw-bold text-red">${{ number_format($product->sale_price, 2) }}</span>
                    </div>
                    @if($product->discount_percentage)
                        <div class="badge bg-red rounded-pill">{{ $product->discount_percentage }}% {{ __('shop.off') }}</div>
                    @endif
                @else
                    <span class="fw-bold fs-5">${{ number_format($product->regular_price, 2) }}</span>
                @endif
            </div>
